...
import { ActorContext, IntelEventType, NormalizedEvent, RawRuntimeEvent, TelemetryTier } from './normalized-event';
...
export class NormalizedEventBus {
  ...
  public ingest(raw: RawRuntimeEvent, actor: ActorContext, states?: { before?: string; after?: string }): NormalizedEvent | undefined {
    ...
    const event: NormalizedEvent = {
      ...
      lifecycle: ['captured', 'normalized', 'deduplicated'],
      payload: raw.payload,
    };
  ...
  public timeline(): NormalizedEvent[] {
    return [...this.events].sort((a, b) => (a.ts - b.ts) || (a.seq - b.seq));
  }

  public markReplayReady(chainId: string): void {
    for (const e of this.events) {
      if (e.chainId === chainId && !e.lifecycle.includes('replay_ready')) e.lifecycle.push('grouped', 'replay_ready');
    }
  }
  ...
  private classifyTier(type: IntelEventType): TelemetryTier {
    ...
    if (['click', 'input', 'form_submit', 'route_transition', 'modal_open', 'modal_close'].includes(type)) return 'WORKFLOW';
    if (['navigation', 'api_request', 'api_response', 'websocket', 'sse'].includes(type)) return 'STRUCTURAL';
    if (['mutation', 'storage_access', 'cookie_mutation', 'indexeddb_access'].includes(type)) return 'DEBUG';
    return 'NOISE';
  }
