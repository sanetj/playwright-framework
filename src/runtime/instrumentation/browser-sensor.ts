export const browserSensorScript = `(() => {
  ...
  window.__intel = { emit, queue, version: '2.0.0' };
  ...
  const wrapHistory = (name) => { const o = history[name]; history[name] = function(...a){ const r = o.apply(this, a); emit('route_transition',{kind:name,to:location.href}); return r; }; };
  ...
  const mo = new MutationObserver((muts) => emit('mutation',{count:muts.length,sample:muts.slice(0,4).map(m=>({t:m.type,n:m.target?.nodeName}))}));
  ...
  indexedDB.open = function(name, version){ emit('indexeddb_access',{op:'open',name,version}); return idbOpen(name, version); };
  ...
  if (cookieDesc && cookieDesc.set && cookieDesc.get) {
    Object.defineProperty(document, 'cookie', {
      ...
      set(v) { emit('cookie_mutation', { preview: String(v).slice(0, 120) }); return cookieDesc.set.call(document, v); },
    });
  }

  const oPM = window.postMessage.bind(window);
  window.postMessage = (message, targetOrigin, ...rest) => { emit('postMessage',{targetOrigin,preview:String(message).slice(0,80)}); return oPM(message,targetOrigin,...rest); };

  const OWS = window.WebSocket;
  window.WebSocket = function(url, protocols){ const ws = new OWS(url, protocols); emit('websocket_open',{url}); ws.addEventListener('message',(ev)=>emit('websocket_message',{url,size:String(ev.data).length})); ws.addEventListener('send',()=>emit('websocket_send',{url})); return ws; };

  const OES = window.EventSource;
  if (OES) window.EventSource = function(url, config){ const es = new OES(url, config); emit('eventsource_open',{url}); es.addEventListener('message',(ev)=>emit('eventsource_message',{url,size:String(ev.data).length})); return es; };

  const addEvt = EventTarget.prototype.addEventListener;
  EventTarget.prototype.addEventListener = function(type, listener, options){
    if (['click','submit','change','input'].includes(String(type))) emit('event_listener',{type,target:(this && this.constructor && this.constructor.name) || 'unknown'});
    return addEvt.call(this, type, listener, options);
  };
